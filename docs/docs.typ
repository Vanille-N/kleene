...
#show: mantys(
  ...
  abstract: [
    ...
    that places a particular focus on *accessibility*, *modularity*,
    *quality of error messages*, and *unit testing*.
    Parsers generated by KLEENE can be modified on the fly
...
- #universe("jiexi") was recently released, employing a completely different approach
  ...
  is based on shift-reduce automata and uses an external tool via WASM.
  This will result in a completely different tradeoff in terms of performance,
  quality of error messages, customizability, etc.
